...
import { setFarmaciaSession } from "@/lib/sessionManager";
...
export default function LoginFarmaciaPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Buscar credenciales en tabla farmacias_credenciales
      const { data: credencial, error: credError } = await supabase
        .from("farmacias_credenciales")
        .select("id, farmacia_id, email, password_hash")
        .eq("email", email)
        .single();
      ...
      const isPasswordValid = await comparePassword(password, credencial.password_hash);
      ...
      // 5. Redirigir a dashboard de farmacia
      router.push("/farmacia/dashboard");
    } catch (err: any) {
      console.error("Error en login farmacia:", err);
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <p className="text-sm text-gray-600 mt-2">
            Acceso exclusivo para farmacias registradas
          </p>
        </CardHeader>
        <CardContent>
